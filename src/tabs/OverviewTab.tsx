...
export function OverviewTab({ summary, queues, agents, tenants, permissions, now, session, agentGroups, incomingCalls }: OverviewTabProps) {
  ...
  const ringingAgents = useMemo(() => agents.filter((a) => a.status === 'ringing'), [agents]);
  ...
  const queueCallDetails = useMemo(() => {
    ...
    for (const queue of queues) {
      ...
      if (incomingForQueue) {
        map.set(queue.id, {
          detail: buildLiveOrIncomingDetail('incoming', incomingForQueue, null, queues, tenants, incomingCalls || [], now),
      ...
      const ringingAgentForQueue = ringingAgents.find((agent) => agent.queueIds.includes(queue.id));
      if (ringingAgentForQueue) {
        map.set(queue.id, {
          detail: buildLiveOrIncomingDetail('incoming', null, ringingAgentForQueue, queues, tenants, incomingCalls || [], now),
          hint: 'Click to open the incoming caller details.',
        });
        continue;
      }
      ...
      const liveAgentForQueue = liveAgents.find((agent) => agent.queueIds.includes(queue.id));
      if (liveAgentForQueue) {
        map.set(queue.id, {
          detail: buildLiveOrIncomingDetail('live', null, liveAgentForQueue, queues, tenants, incomingCalls || [], now),
      ...
      const anyBroadcastForQueue = (incomingCalls || []).find((call) => call.queueId === queue.id);
      if (anyBroadcastForQueue) {
        map.set(queue.id, {
          detail: buildLiveOrIncomingDetail('incoming', anyBroadcastForQueue, null, queues, tenants, incomingCalls || [], now),
          hint: 'Click to open the incoming caller details.',
        });
      }
    }
    ...
    <div className="cc-fade-in space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
          {queues.filter((q) =>
            permissions.allowedQueueIds.length === 0 || permissions.allowedQueueIds.includes(q.id)
          ).map((q) => {
            ...
            return (
              <QueueSummaryCard
                ...
                onClick={queueDetail ? () => setSelectedCall(queueDetail.detail) : undefined}
              />
            );
          })}
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {permissions.canViewTenantNames && <TableHead>Client</TableHead>}
              ...
              <TableBody>
                {liveAgents.map((a) => {
                  const queueColor = queues.find((q) => a.queueIds.includes(q.id))?.color || 'var(--cc-color-cyan)';

                  return (
                    <TableRow
                      key={a.id}
                      className="cursor-pointer transition-colors hover:bg-slate-50"
                      onClick={() => setSelectedCall(buildLiveCallSnapshot({
                        agent: a,
                        queues,
                        tenants,
                        incomingCall: findIncomingCallForAgent(a, incomingCalls || []),
                        now,
                      }))}
                    >
                      <TableCell className="font-medium text-slate-900">{a.name}</TableCell>
                      {permissions.canViewTenantNames && (
                        <TableCell className="text-muted-foreground">{a.tenantName}</TableCell>
                      )}
                      <TableCell>
                        <Badge
                          variant="outline"
                          className="rounded-full border-0 px-2.5 py-1 text-[11px] font-semibold"
                          style={{ color: queueColor, background: `${queueColor}18` }}
                        >
                          {a.queueName}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell className="font-mono text-xs">
                        {formatPhone(a.currentCaller || findIncomingCallForAgent(a, incomingCalls || [])?.callerNumber || '')}
                      </TableCell>
                      <TableCell className="font-mono text-xs font-semibold text-rose-600">
                        <span className="inline-flex items-center">
                          <LiveDot />
                          {a.callStartTime ? formatDuration(now - a.callStartTime) : '—'}
                        </span>
                      </TableCell>
                    </TableRow>
                  );
...
function findIncomingCallForAgent(agent: Agent, incomingCalls: IncomingCall[]): IncomingCall | null {
