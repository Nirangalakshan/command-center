...
import {
  ...
  onAuthStateChanged as firebaseOnAuthStateChange,
...
export function useAuth(): AuthState {
  ...
  useEffect(() => {
    ...
    const unsubscribeFirebase = firebaseOnAuthStateChange(
      auth,
      async (firebaseUser) => {
        // Ignore the background service account used for data querying
        if (
          firebaseUser &&
          firebaseUser.email === import.meta.env.VITE_FIREBASE_AGENT_EMAIL
        ) {
          return;
        }
        ...
        if (firebaseUser) {
          ...
          try {
            const agentDoc = await getDoc(
              doc(db, "call_center_agents", firebaseUser.uid),
            );
            if (agentDoc.exists()) {
              ...
              await updateDoc(doc(db, "call_center_agents", firebaseUser.uid), {
                status: "available",
              }).catch(console.warn);
              setSession(userSession);
            } else {
  ...
  const signIn = useCallback(async (email: string, password: string) => {
      ...
      setTimeout(async () => {
        try {
          ...
          const userSession: UserSession = {
            ...
            allowedQueueIds: [],
            displayName:
              profileRes.data?.display_name || authData.user.email || "",
          };
    ...
    try {
      ...
      const agentDoc = await getDoc(
        doc(db, "call_center_agents", fbCred.user.uid),
      );
  ...
  const signOut = useCallback(async () => {
    if (session?.role === "agent") {
      ...
      if (auth.currentUser) {
        await updateDoc(doc(db, "call_center_agents", session.userId), {
          status: "offline",
        }).catch(() => {});
      }
    }
