...
import { CalendarDays, ChevronLeft, ChevronRight, Coffee, LogIn, LogOut, Timer } from "lucide-react";
...
function statusDotColor(status: string): string {
  switch (status) {
    case "working": return "var(--cc-color-green)";
    case "on_break": return "var(--cc-color-amber)";
    default: return "var(--cc-color-slate, #94a3b8)";
  }
}
...
function weekDayKeys(dateKey: string): string[] {
  ...
  return Array.from({ length: 7 }, (_, i) => addAustralianCalendarDays(monday, i));
}
...
export function AgentAttendanceCard({ session, now }: AgentAttendanceCardProps) {
  ...
  const [weekEvents, setWeekEvents] = useState<Record<string, AgentAttendanceEventRow[]>>({});
  ...
  const reloadWeek = useCallback(async () => {
    ...
    try {
      ...
      days.forEach((dk, i) => { map[dk] = results[i]; });
      setWeekEvents(map);
    } catch (e: unknown) {
  ...
  const weekRows: WeekRow[] = useMemo(() => {
    return days.map((dk, idx) => {
      ...
      const { workedMs, breakMs } = computeWorkedAndBreakMs(effectiveEvs, capMs);
  ...
  return (
    <Card className="cc-fade-in border-border/80 bg-gradient-to-br from-white via-white to-emerald-50/30 shadow-sm">
      <CardHeader className="gap-3 border-b border-border/70 pb-4">
            ...
            {tab === "week" && (
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  ...
                  onClick={() => setWeekAnchor((k) => addAustralianCalendarDays(k, -7))}
                >
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                <span className="font-mono text-sm font-semibold text-slate-900">
                  {formatAustralianDayShort(days[0])} – {formatAustralianDayShort(days[6])}
                </span>
                <Button
                  type="button"
                  variant="outline"
        ...
        {tab === "day" && isViewingToday && (shiftStartedAt || breakStartedAt) && (
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="hover:bg-transparent">
                        ...
                        <TableHead className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Clock in</TableHead>
                        <TableHead className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Clock out</TableHead>
                        <TableHead className="text-right font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Break</TableHead>
                        <TableHead className="text-right font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Working hours</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {segments.map((seg, i) => (
          ...
          <>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-emerald-200/70 bg-emerald-50/40 p-4 shadow-sm">
                ...
                <div className="mt-2 font-mono text-2xl font-semibold text-emerald-700">
                  {weekLoading ? "…" : weekTotalWorkedMs > 0 ? formatDuration(weekTotalWorkedMs) : "0:00"}
                </div>
              </div>
              <div className="rounded-xl border border-amber-200/70 bg-amber-50/40 p-4 shadow-sm">
                ...
                <div className="mt-2 font-mono text-2xl font-semibold text-amber-700">
                  {weekLoading ? "…" : weekTotalBreakMs > 0 ? formatDuration(weekTotalBreakMs) : "0:00"}
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border/80 bg-white shadow-sm">
              <div className="border-b border-border/60 bg-slate-50/80 px-4 py-2.5">
                <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
                  Daily breakdown · {formatAustralianDayShort(days[0])} – {formatAustralianDayShort(days[6])}
                </div>
              </div>
              {weekLoading ? (
                <p className="p-4 text-center text-sm text-muted-foreground">Loading…</p>
                ...
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow className="hover:bg-transparent">
                        <TableHead className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Day</TableHead>
                        <TableHead className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Date</TableHead>
                        <TableHead className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground whitespace-nowrap">Clock in</TableHead>
                        <TableHead className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground whitespace-nowrap">Clock out</TableHead>
                        <TableHead className="text-right font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Break</TableHead>
                        <TableHead className="text-right font-mono text-[10px] uppercase tracking-wider text-muted-foreground whitespace-nowrap">Working hrs</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {weekRows.map((row) => (
                        ...
                        >
                          ...
                          <TableCell className="text-right font-mono text-sm font-medium text-emerald-800">
                            {row.workedMs > 0 ? formatDuration(row.workedMs) : "—"}
                          </TableCell>
                        </TableRow>
                      ))}
                      {/* Totals row */}
                      <TableRow className="border-t-2 border-border/80 bg-slate-50/60 font-medium hover:bg-slate-50/60">
                        <TableCell colSpan={4} className="text-right text-sm text-slate-700">
                          Week totals
                        </TableCell>
                        <TableCell className="text-right font-mono text-sm text-amber-800/90">
                          {weekTotalBreakMs > 0 ? formatDuration(weekTotalBreakMs) : "0:00"}
                        </TableCell>
                        <TableCell className="text-right font-mono text-sm text-emerald-800">
                          {weekTotalWorkedMs > 0 ? formatDuration(weekTotalWorkedMs) : "0:00"}
                        </TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
            ...
            <p className="text-xs text-muted-foreground">
              Week runs Mon–Sun (Australia/Melbourne time). Today's hours update live.
              {!isCurrentWeek && " Navigate to the current week to clock in or out."}
            </p>
          </>
        )}
      </CardContent>
